...
public:
			...
			for (int vi = 0; vi < vD; vi++) {
				...
				if ((p01 - p10).sqr() < (p00 - p11).sqr()) {
					if (p10 != p00 && p10 != p01 && p00 != p01)
						T.push_back(triangle{ p10, p00, p01 }), NTrigs++;
					if (p01 != p11 && p01 != p10 && p11 != p10)
						T.push_back(triangle{ p01, p11, p10 }), NTrigs++;
				}
				else {
					if (p11 != p10 && p11 != p00 && p10 != p00)
						T.push_back(triangle{ p11, p10, p00 }), NTrigs++;
					if (p00 != p01 && p00 != p11 && p01 != p11)
						T.push_back(triangle{ p00, p01, p11 }), NTrigs++;
				}
			}
		}
		return NTrigs;
	...
	int param2trigs(std::vector<triangle> &T,
		vec3 translate = vec3(0.), double scale = 1.) const {
		T.reserve(T.size() + 2 * uD*vD);
		std::vector<vec3> p;
