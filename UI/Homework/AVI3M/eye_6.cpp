...
vec3 calcCol(vec3 ro, vec3 rd, uint32_t &seed) {
	...
	for (int iter = 0; iter < 20; iter++) {
		...
		if (min_t > 0.) {
			...
			p.x -= .5; col = p.sqr() + (p.x + .5)*abs(p.y) < .25 ? vec3(0.9, 0.8, 0.7) : vec3(0.7, 0.7, 0.8);
			min_n = vec3(0, 0, 1);
			intersect_id = 0;
		}
		else {
...
//const vec3 CamP = vec3(13.686091, -2.556056, 4.739081), ScrO = vec3(0.816673, -3.789772, -0.955875), ScrA = vec3(1.259577, 6.921911, 0.000000), ScrB = vec3(-1.106667, 0.201380, 3.911749);
const vec3 CamP = vec3(12.924644, -4.978020, 4.998377), ScrO = vec3(0.151197, -3.928795, -0.944236), ScrA = vec3(2.478454, 6.584577, 0.000000), ScrB = vec3(-1.125739, 0.423731, 3.888471);
