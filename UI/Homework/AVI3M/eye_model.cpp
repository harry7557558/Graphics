...
double eyes(vec3 p) {
	...
	auto one_eye = [&](vec3 p) {
		...
		return max(min(k, kl), -iris(3. * (p - vec3(0.35, 0, 0))));  // iris inside eyeball
		return min(min(k, kl), iris(3. * (p - vec3(0.6, 0, 0))));  // iris outside eyeball
	};
...
void subdivide_trigs_refine(double(*fun)(vec3), vec3* P, int N, double max_move) {
	/*
		...
		warn: may be numerically unstable
	*/
	// numerical gradient
	auto ngrad = [&](vec3 p) {
		double eps = 1e-5;
		return vec3(
			fun(p + vec3(eps, 0, 0)) - fun(p - vec3(eps, 0, 0)),
			fun(p + vec3(0, eps, 0)) - fun(p - vec3(0, eps, 0)),
			fun(p + vec3(0, 0, eps)) - fun(p - vec3(0, 0, eps))
		) * (.5 / eps);
	};
	// Newton iteration
	for (int i = 0; i < N; i++) {
		...
		// max_move: prevent endless loop/recursion caused by numerical instability
		if (length(P[i] - p) < max_move) P[i] = p;
	}
}
void subdivide_trigs_splitTriangle(double(*fun)(vec3), triangle T, std::vector<triangle> &Ts, double max_length, int recurse_remain = 20) {
	...
	*/
	...
	ivec2 Edges[3] = { ivec2(0,1), ivec2(1,2), ivec2(2,0) };
	double lengths[3];
	for (int i = 0; i < 3; i++) lengths[i] = length(Vertice[Edges[i].x] - Vertice[Edges[i].y]);
	int code = 0b001 * int(lengths[0] > max_length) + 0b010 * int(lengths[1] > max_length) + 0b100 * int(lengths[2] > max_length);
	if (code == 0 || recurse_remain < 0) {
		Ts.push_back(T);
		return;
	}
#if 0
	// intended to prevent very long triangles, does not work
	double lengths_max = std::max({ lengths[0], lengths[1], lengths[2] });
	double lengths_min = std::min({ lengths[0], lengths[1], lengths[2] });
	if (code == 7 && lengths_max  > 1.5 * lengths_min) {
		double avr_length = 0.5*(lengths_max + lengths_min);
		code = 0b001 * int(lengths[0] > avr_length) + 0b010 * int(lengths[1] > avr_length) + 0b100 * int(lengths[2] > avr_length);
	}
#endif
	for (int i = 0; i < 3; i++) {
		if ((1 << i) | code) {
			Vertice[i + 3] = 0.5*(Vertice[Edges[i].x] + Vertice[Edges[i].y]);
			subdivide_trigs_refine(fun, &Vertice[i + 3], 1, max_length);
		}
	}
	const static int Trigs[8][13] = {
		{0, 1, 2, -1},
		{0, 3, 2, 3, 1, 2, -1},
		{0, 1, 4, 0, 4, 2, -1},
		{0, 3, 2, 3, 4, 2, 3, 1, 4, -1},
		...
		{0, 3, 5, 3, 1, 5, 5, 1, 2, -1},
		{0, 1, 5, 5, 1, 4, 5, 4, 2, -1},
		{0, 3, 5, 3, 1, 4, 5, 4, 2, 3, 4, 5, -1}
	};
	for (int i = 0; Trigs[code][i] != -1; i += 3) {
		subdivide_trigs_splitTriangle(fun, triangle{
			Vertice[Trigs[code][i]],
			Vertice[Trigs[code][i + 1]],
			Vertice[Trigs[code][i + 2]]
			}, Ts, max_length, recurse_remain - 1.);
	}
}
void subdivide_trigs(double(*fun)(vec3), std::vector<triangle> T, std::vector<triangle> &Res, double max_length) {
	// move points closer to the isosurface
	subdivide_trigs_refine(fun, (vec3*)&T[0], 3 * T.size(), max_length);
	// split recursively
	Res.clear();
	for (int i = 0, TN = T.size(); i < TN; i++) {
		subdivide_trigs_splitTriangle(fun, T[i], Res, max_length);
	}
}
...
int main(int argc, char* argv[]) {
	...
	vec3 bound(1., 3., 2.);
	marching_cube(eyes, T, -bound, bound, ivec3(120.*bound));
	subdivide_trigs(eyes, T, T, 1. / (120. / 2.));
