...
vec3 calcCol(vec3 ro, vec3 rd, uint32_t &seed) {
	...
	for (int iter = 0; iter < 20; iter++) {
		...
		if (min_t > 0.) {
			...
			//col *= exp(-0.01*p.sqr());
			col = (max(abs(p.x) - 0.618, abs(p.y) - 1.) < 0. ? vec3(0.85, 0.75, 0.75) : vec3(0.65, 0.6, 0.55))*exp(-0.001*p.sqr());
			...
			//p.x -= .5; col = p.sqr() + (p.x + .5)*abs(p.y) < .25 ? vec3(0.9, 0.8, 0.7) : vec3(0.7, 0.8, 0.7);
			min_n = vec3(0, 0, 1);
			intersect_id = 0;
		}
		else {
		...
		if (intersectScene(Eyeball, ro, rd, t, n) && t < min_t) {
			min_t = t, min_n = n;
			col = isInside ? exp(-vec3(0., .5, 1.)*min_t) : vec3(1.);
			intersect_id = 1;
		}
		...
		if (intersect_id == 0) {
			...
			//rd = (int(floor(ro.x + ro.y)) ^ int(floor(ro.x - ro.y))) & 1 ? rd - min_n * (2.*dot(rd, min_n)) : randdir_cosWeighted(min_n, seed);
			rd = mix(rd - min_n * (2.*dot(rd, min_n)), randdir_cosWeighted(min_n, seed), .5);  // not quite physical
		}
		else {
		...
		for (int k = beg; k < end; k += step) {
			...
			for (int u = 0; u < N; u++) {
				...
				if (1) {
					...
					// camera with dof, I don't think I implemented it correctly
					vec2 uv = 0.05*cossin(2.*PI*rand01(seed));
					vec3 cp = scrPos(vec2(i + rand01(seed), j + rand01(seed)));
					vec3 pos = CamP + uv.x*normalize(ScrA) + uv.y*normalize(ScrB);
					vec3 d = cp - pos;
					col += calcCol(pos, normalize(d), seed);
				}
			}
