// A binary STL and PLY 3D model viewer for Windows
...
void calcBoundingBox() {
	...
	for (int i = 0; i < VN; i++) {
		if (!isnan(dot(V[i], vec3f(1.f)))) BMin = pMin(BMin, V[i]), BMax = pMax(BMax, V[i]);
	}
}
...
void calcVolumeCenterInertia() {
	...
	for (int i = 0; i < TN; i++) {
		// I may have a bug
		vec3 a = vec3(V[T[i][0]]), b = vec3(V[T[i][1]]), c = vec3(V[T[i][2]]);
		if (TreatAsSolid) {
...
double rayIntersect(vec3f ro, vec3f rd) {
	float mt = INFINITY;
	for (int i = 0; i < TN; i++) {
		vec3f a = vec3f(V[T[i][0]]), b = vec3f(V[T[i][1]]), c = vec3f(V[T[i][2]]);
...
void render() {
	...
	for (int i = 0; i < VN; i++) V_transformed[i] = TF(V[i]);
	...
	for (int i = 0; i < TN; i++) {
		vec3f At = V_transformed[T[i][0]], Bt = V_transformed[T[i][1]], Ct = V_transformed[T[i][2]];
	...
	// grid and axes
	{
		if (showGrid) {
			...
			if (showMajorMinorGrids) {
				double range = VN > 0 ? std::max({ BMax.x, BMax.y, -BMin.x, -BMin.y }) : NAN;
	...
	WCHAR text[1024];
	if (isnan(readedValue.sqr())) {  // information
		int lastslash = 0;
...
void MouseWheel(int _DELTA) {
	...
	if (Ctrl) Center.z += 0.1 * _DELTA / Unit;
		...
		if (Shift) {
			for (int i = 0; i < VN; i++) V[i].z *= sc;
		}
		else {
			for (int i = 0; i < VN; i++) V[i] = (V[i] - C)*sc + C;
		}
