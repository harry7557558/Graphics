# 64x64 GAN trained on FFHQ, and something interesting.
# The model doesn't seem to learn data distribution. Run it to see duplicates.
...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        layers = [
            ...
            # dims[2] x 16x16
            nn.Upsample(scale_factor=2),
            nn.Conv2d(dims[2], dims[3], 5, 1, 2, bias=False),
            nn.LeakyReLU(0.1, inplace=True),
            # dims[3] x 32x32
            nn.Upsample(scale_factor=2),
            nn.Conv2d(dims[3], 3, 5, 1, 2, bias=False),
            nn.Sigmoid()
            # 3 x 64x64
        ]
