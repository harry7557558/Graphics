...
# trained on FFHQ, use upsample instead of convtrans
...
LATENT = 24
IMAGE_PATH = "progress-32x32"
MODEL_G_PATH = "ffhq-32x32-g.pth"
MODEL_D_PATH = "ffhq-32x32-d.pth"
...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        layers = [
            ...
            # 16 x 1x1
            nn.Conv2d(LATENT, dims[0], 4, 1, 3, bias=False),
            ...
            # dims[2] x 16x16
            nn.Upsample(scale_factor=2),
            nn.Conv2d(dims[2], 3, 5, 1, 2, bias=False),
            nn.Sigmoid()
            # 3 x 32x32
        ]
...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # dims[2] x 4x4
            nn.Conv2d(dims[2], 1, 4, 1, 0, bias=False),
            nn.Flatten(),
            nn.Sigmoid()
        )
...
def main_plot():
    ...
    def images(t: float):
        z = tensor(noise(t).astype(np.float32))
        generated = net_g(z)
