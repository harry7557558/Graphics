...
function Activate(grid_size, tile_size, n_in) {
    if (typeof grid_size == 'number') {
        this.grid_size_x = grid_size;
        this.grid_size_y = grid_size;
    }
    ...
    this.forward = function(buffer_in, buffer_out) {
        let gl = renderer.gl;
        let program = renderer.programActivate;
        gl.useProgram(program);
        gl.viewport(0, 0, this.grid_size_x*this.tile_size, this.grid_size_y*this.tile_size);
        for (var i = 0; i < this.n_out; i += 4) {
            gl.bindFramebuffer(gl.FRAMEBUFFER, buffer_out.imgs[Math.floor(i/4)].framebuffer);
...
async function drawScene() {
    ...
    for (var gi = 0; gi < nx; gi++) {
        for (var gj = 0; gj < ny; gj++) {
            ...
            var latent = new Array(nz).fill(0.0);
            for (var i = 0; i < nz; i++) {
                var t1 = t + hash(i);
                var i0 = Math.floor(t1), i1 = i0 + 1.0, tf = t1 - i0;
                var u1 = hash(i0*nz+i), u2 = hash(i1*nz+i);
                var u = u1 + (u2-u1) * tf;
                var v1 = hash(i0*nz+i+0.5), v2 = hash(i1*nz+i+0.5);
                var v = v1 + (v2-v1) * tf;
            ...
            // linear - (nz) => (1024)
            var layer1js = new Array(1024).fill(0.0);
            let w01 = renderer.w01_1024_64;
            let b01 = renderer.b01_1024;
            for (var i = 0; i < 1024; i++) {
                layer1js[i] = b01[i];
                for (var j = 0; j < nz; j++)
                    layer1js[i] += latent[j] * w01[i*nz+j];
            }
            // activate
            for (var i = 0; i < 1024; i++) {
                var c = layer1js[i];
                layer1js[i] = c / (1.0+Math.exp(-c));
            }
            // add to data
            for (var i = 0; i < 64; i++)
                for (var y = 0; y < 4; y++)
    ...
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, r.layer6.imgs[0].texture);
...
// load model
function loadModel() {
    loadWeightTexture("w01_1024_64", "w01");
    loadWeightTexture("b01_1024", "b01");
    loadWeightTexture("w02_64_64_3_3", "w02");
    ...
    loadWeightTexture("w06_64_3_4_4", "w06");
    loadWeightTexture("b06_3", "b06");
}
...
function updateModel() {
    ...
    let ny = Math.floor(canvas.height / 64);
    let n = [nx, ny];
    r.layer1 = new CNNBuffer(64, 4*nx, 4*ny);
    r.conv2 = new Conv2d311(n, 4, 64, 64, r.w02_64_64_3_3, r.b02_64);
    r.layer2 = new CNNBuffer(64, 4*nx, 4*ny);
    r.relu2 = new Activate(n, 4, 64);
    ...
    r.relu5 = new Activate(n, 32, 64);
    r.layer5a = new CNNBuffer(64, 32*nx, 32*ny);
    r.conv6 = new ConvTranspose2D421(n, 32, 64, 3, r.w06_64_3_4_4, r.b06_3);
    r.layer6 = new CNNBuffer(3, 64*nx, 64*ny);
}
...
window.onload = function () {
    ...
    renderer.fsActivate = loadShaderSource("activate.glsl");
    ...
    try {
        ...
        renderer.programActivate = createShaderProgram(renderer.vsSource, renderer.fsActivate);
