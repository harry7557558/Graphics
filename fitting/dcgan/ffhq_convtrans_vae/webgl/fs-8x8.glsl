#version 300 es
precision highp float;

uniform sampler2D uSrc;
uniform sampler2D uWeights;
uniform sampler2D uBiases;


int getIRes() {
    ivec2 p = ivec2(gl_FragCoord.xy) % 64;
    int i = p.y * 64 + p.x;
    return i * 4;
}
float getSrc(int i) {
    ivec2 p0 = (ivec2(gl_FragCoord.xy)/64)*64;
    vec4 r = texelFetch(uSrc, p0+ivec2((i/4)%64, (i/4)/64), 0);
    return i%4==0 ? r.x : i%4==1 ? r.y : i%4==2 ? r.z : r.w;
}
float getW(int i) {
    int w = textureSize(uWeights, 0).x;
    vec4 r = texelFetch(uWeights, ivec2((i/4)%w, (i/4)/w), 0);
    return i%4==0? r.x : i%4==1 ? r.y : i%4==2 ? r.z : r.w;
}
float getB(int i) {
    int w = textureSize(uBiases, 0).x;
    vec4 r = texelFetch(uBiases, ivec2((i/4)%w, (i/4)/w), 0);
    return i%4==0? r.x : i%4==1 ? r.y : i%4==2 ? r.z : r.w;
}

out vec4 fragColor;
void main() {

    int i0 = getIRes();
    if (i0 >= 4096) discard;

    float v[4];
    for (int i = i0; i < i0 + 4; i++) {
        int chr = i / 64;  // res channel
        int ri = (i-chr*64) / 8, rj = i % 8;  // res pixel
        float s = 0.0;
        for (int chs = 0; chs < 32; chs++) {  // src channel
            int w0 = (chs * 64 + chr) * 16;
            for (int ci = 0; ci < 4; ci++) for (int cj = 0; cj < 4; cj++) {  // filter
                int si = ri + ci - 2, sj = rj + cj - 2;
                if (si >= 0 && si < 16 && si % 2 == 0
                    && sj >= 0 && sj < 16 && sj % 2 == 0) {
                    si /= 2, sj /= 2;  // reverse stride
                    float wv = getW(w0 + (3-ci) * 4 + (3-cj));
                    float sv = getSrc(chs * 16 + (si * 4 + sj));
                    s += wv * sv;
                }
            }
        }
        s += getB(chr);
        v[i-i0] = max(0.1*s, s);
    }

    fragColor = vec4(v[0], v[1], v[2], v[3]);

}
