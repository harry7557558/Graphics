...
float OrenNayarBRDF(float sigma, vec3f n, vec3f wi, vec3f wo) {
	// not optimized
	vec3f u = normalize(cross(n, vec3f(1.2345f, 2.3456f, -3.4561f)));
	vec3f v = cross(u, n);
	wi = vec3f(dot(wi, u), dot(wi, v), dot(wi, n));
	wo = vec3f(dot(wo, u), dot(wo, v), dot(wo, n));
	float s2 = sigma * sigma;
	float A = 1.0f - s2 / (2.0f*(s2 + 0.33f));
	float B = 0.45f*s2 / (s2 + 0.09f);
	float theta_i = acos(wi.z), phi_i = atan2(wi.y, wi.x);
	float theta_o = acos(wo.z), phi_o = atan2(wo.y, wo.x);
	return (A + B * max(0.0f, cos(phi_i - phi_o))*sin(max(theta_i, theta_o))*tan(min(theta_i, theta_o))) / PIf;
}
...
vec3f mainRender(vec3f ro, vec3f rd, uint32_t &seed) {
	...
	for (int iter = 0; iter < 64; iter++) {
		...
		if (material == diffuse) {  // diffuse
			vec3f wi = -rd;
			vec3f wo = sampleCosineWeighted(min_n, seed);
			m_col *= PIf * OrenNayarBRDF(0.5f, min_n, wi, wo);
			rd = wo;
		}
		if (dot(rd, min_n) < 0.0) {
			is_inside ^= 1;
