...
vec3 calcCol(vec3 ro, vec3 rd, uint32_t &seed) {
	...
	for (int iter = 0; iter < 64; iter++) {
		...
		if (0) {
			// steel ball
			t = min_t;
			if (intersectSphere(vec3(0.0), 1.0, ro, rd, t, n)) {
				min_t = t, min_n = n;
				col = vec3(0.9);
				intersect_id = 1;
			}
		...
		// stained glass balls
		if (1) {
			const double dist = 2.0, abs = 2.0;
			const vec3 poses[3] = { vec3(dist,0,0), vec3(0,dist,0), vec3(0,0,dist) };
			const vec3 absorbs[3] = { vec3(0.0,abs,abs), vec3(abs,0.0,abs), vec3(abs,abs,0.0) };
			for (int i = 0; i < 3; i++) {
				t = min_t;
				if (intersectSphere(poses[i], 0.2, ro, rd, t, n)) {
					min_t = t, min_n = n;
					col = is_inside ? exp(-absorbs[i] * min_t) : vec3(1.0);
					intersect_id = 2;
				}
			}
		}
		...
		ro = ro + rd * min_t;
		if (intersect_id == 0) {  // diffuse
			rd = randdir_cosWeighted(min_n, seed);
		}
		else if (intersect_id == 1) {  // steel ball
			rd = rd - min_n * (2.*dot(rd, min_n));
		}
		else if (intersect_id == 2) {  // glass ball
			vec2 refl_index = is_inside ? vec2(1.5, 1.0) : vec2(1.0, 1.5);
			rd = randdir_Fresnel(rd, n, refl_index.x, refl_index.y, seed);
		}
		if (dot(rd, min_n) < 0.0) {
			is_inside ^= 1;
		}
