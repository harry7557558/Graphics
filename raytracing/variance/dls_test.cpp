...
public:
	...
	SceneObject(SceneObjectType type, Intersector intersector, DLS_Sampler dls_sampler) {
		this->type = type;
		this->intersector = intersector;
		if (this->type == sotLight) this->dls_sampler = dls_sampler;
	}
...
// bruteforce Monte Carlo integral
vec3 calcCol(vec3 ro, vec3 rd, uint32_t &seed, bool dls) {
	...
	for (int iter = 0; iter < 64; iter++) {
		...
		SceneObject::SceneObjectType intersect_type = SceneObject::sotNone;
		...
		for (int id = 0; id < (int)scene.size(); id++) {
			...
			if (object.intersector(ro, rd, io)) {
				intersect_type = object.type;
				intersect_id = id;
			}
		}

		if (intersect_type == SceneObject::sotNone) {
			return t_col;
		}
		...
		if (intersect_type == SceneObject::sotDiffuseLambert) {
			...
			if (dls) {
				for (int dls_light_id = 0; dls_light_id < (int)scene.size(); dls_light_id++) {
					SceneObject light = scene[dls_light_id];
					if (light.type == SceneObject::sotLight) {
						...
						if (dot(dlso.rd, n) > 0.0) {
							...
							int dls_intersect_id = -1;
							vec3 light_col = vec3(0.0);
							for (int id = 0; id < (int)scene.size(); id++) {
								SceneObject object = scene[id];
								if (object.intersector(ro + 1e-6*dlso.rd, dlso.rd, io)) {
									dls_intersect_id = id;
									light_col = io.color;
								}
							}

							if (dls_intersect_id == dls_light_id) {
								light_col *= dlso.weight;
	...
	Render_Exec([](int beg, int end, int step, bool* sig) {
		...
		for (int k = beg; k < end; k += step) {
			...
			for (int u = 0; u < N; u++) {
				...
				vec3 ro = CamP + 1.0*rd;
				//col += calcCol(ro, rd, seed, false);
				col += calcCol(ro, rd, seed, true);
				//col += calcCol(ro, rd, seed, true) - calcCol(ro, rd, seed, false);
			}
...
void testConsistency() {
	...
	std::function<vec3(uint32_t)> without_dls = [&](uint32_t seed) {
		return calcCol(ro, rd, seed, false);
	};
	std::function<vec3(uint32_t)> with_dls = [&](uint32_t seed) {
		return calcCol(ro, rd, seed, true);
	};
