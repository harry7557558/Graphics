...
bool intersectScene(BVH* Scene, vec3 ro, vec3 rd, double &mt, vec3 &n) {
	double t = intBoxC(Scene->R, ro - Scene->C, vec3(1.0) / rd);
	if (!(t < mt)) return false;
	if (!isnan(t)) {
		BVH_Triangle* obj = 0;
		rayIntersectBVH(Scene, ro, rd, vec3(1.0) / rd, mt, obj);
		if (obj) {
			n = normalize(obj->n);
			return true;
		}
