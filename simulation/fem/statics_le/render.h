...
const char FRAGMENT_SHADER_SOURCE_FONT[] = R"""(#version 330
in vec2 fragmentUv;
out vec3 fragColor;
uniform int i;
...
// the checkboxes to select the value to visualize
// at the top left of the window
struct VisualizationSelector: WindowInput {
    enum VisualizationMode {
        SigmaX, SigmaY, SigmaZ, TauYZ, TauXZ, TauXY,
        SigmaVonMises, SigmaT, SigmaC,
        UX, UY, UZ, UM,
        VEnd
    };
    std::vector<ToggleCheckbox> togglers;
    const DiscretizedStructure* structure;
    ...
    VisualizationSelector(
        const DiscretizedStructure* structure,
        PrecomputedValues* precomputed
    ): structure(structure), precomputed(precomputed) {
        using glm::vec2;
        FontString buttonTexts[] = {
            FontString({ FontCharacters::sigma, FontCharacters::x }),
            FontString({ FontCharacters::sigma, FontCharacters::y }),
            FontString({ FontCharacters::sigma, FontCharacters::z }),
            FontString({ FontCharacters::tau, FontCharacters::y, FontCharacters::z }),
            FontString({ FontCharacters::tau, FontCharacters::x, FontCharacters::z }),
            FontString({ FontCharacters::tau, FontCharacters::x, FontCharacters::y }),
            FontString({ FontCharacters::sigma, FontCharacters::v, FontCharacters::m }),
            FontString({ FontCharacters::sigma, FontCharacters::t }),
            FontString({ FontCharacters::sigma, FontCharacters::c }),
            FontString({ FontCharacters::u, FontCharacters::x }),
            FontString({ FontCharacters::u, FontCharacters::y }),
            FontString({ FontCharacters::u, FontCharacters::z }),
            FontString({ FontCharacters::u }),
        };
        // add options
        const int size = 24, offset = 30;
        vec2 p(10, RenderParams::iResolution.y - size - 10);
        for (int i = 0; i < UX; i++) {
            togglers.push_back(ToggleCheckbox(p, size, buttonTexts[i]));
            p.x += offset;
        }
        p.y -= offset, p.x = 10;
        for (int i = UX; i < VEnd; i++) {
            togglers.push_back(ToggleCheckbox(p, size, buttonTexts[i]));
            p.x += offset;
        }
        togglers[0].toggled = true;
        updateValues();
    }
    ...
    int getToggled() {
        for (int i = 0; i < VEnd; i++)
            if (togglers[i].toggled) return i;
        assert(false);
        return VEnd;
    }
    void updateValues() {
        ...
        int i = getToggled();
        precomputed->values =
            i == SigmaX ? structure->getSigmaX() :
            i == SigmaY ? structure->getSigmaY() :
            i == SigmaZ ? structure->getSigmaZ() :
            i == TauYZ ? structure->getTauYZ() :
            i == TauXZ ? structure->getTauXZ() :
            i == TauXY ? structure->getTauXY() :
            i == SigmaVonMises ? structure->getSigmaVonMises() :
            i == SigmaT ? structure->getSigmaT() :
            i == SigmaC ? structure->getSigmaC() :
            i == UX ? structure->getUX() :
            i == UY ? structure->getUY() :
            i == UZ ? structure->getUZ() :
            i == UM ? structure->getUM() :
            std::vector<float>(structure->N, 0.0f);
        // find max
        std::vector<float> valuesSorted;
        precomputed->isAllPositive = precomputed->isAllNegative = true;
        for (int i = 0; i < structure->N; i++) {
            float v = precomputed->values[i];
            if (v != 0.0f) valuesSorted.push_back(abs(v));
            precomputed->isAllPositive &= (v >= 0.0f);
            precomputed->isAllNegative &= (v <= 0.0f);
        }
        if (valuesSorted.empty()) valuesSorted.push_back(0.0f);
        std::sort(valuesSorted.begin(), valuesSorted.end());
        float maxValue = valuesSorted.back();
        float minValue =
            precomputed->isAllPositive || precomputed->isAllNegative ?
            0.0f : -maxValue;
        precomputed->colors.resize(structure->N);
        for (int i = 0; i < structure->N; i++) {
            float t = 0.5 + 0.5 * precomputed->values[i] / maxValue;
            precomputed->colors[i] = glm::vec4(-1, -1, -1, t);
        }
        precomputed->maxValue = maxValue;
        precomputed->minValue = minValue;
        // find a median measure
        // used to deal with extreme values when rendering
        i = int(0.8 * (valuesSorted.size() - 1));
        float medianValue = glm::clamp(valuesSorted[i],
            0.01f * maxValue, 0.6f * maxValue);
        precomputed->medianValue = medianValue;
        // printf("Median/Max values: %g %f\n", medianValue, maxValue);
    }

    // interactions
    bool isHover() {
        for (int i = 0; i < (int)togglers.size(); i++)
            if (togglers[i].isHover()) return true;
        return false;
    }
    void mouseClick() {
        int it = getToggled();
        for (int i = 0; i < (int)togglers.size(); i++) {
            if (togglers[i].isHover()) {
                togglers[it].toggled = false;
                togglers[i].toggled = true;
                if (i != it) updateValues();
                return;
            }
        }
    }
...
void mainGUI(
    ...
    VisualizationSelector selector(&structure, &precomputed);
