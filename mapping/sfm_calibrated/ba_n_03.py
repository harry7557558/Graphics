...
def exp_so3t(T):
    ...
    theta = np.sqrt(phi.dot(phi)+1e-20)
    n = phi / theta
    nnT = np.outer(n, n)
...
def exp_so3t_jax(T):
    ...
    theta = jnp.sqrt(phi.dot(phi)+1e-20)
    n = phi / theta
    nnT = jnp.outer(n, n)
    n_star = jnp.array([[0.0, -n[2], n[1]], [n[2], 0.0, -n[0]], [-n[1], n[0], 0.0]])
    R = jnp.cos(theta) * jnp.eye(3) + \
        (1.0-jnp.cos(theta)) * nnT + \
        jnp.sin(theta) * n_star
    return R, t
...
def bundle_adjustment_01(poses, points, observations):
    ...
    # function
    def fun_jax_0(so3t, point_3d, point_2d):
        R, t = exp_so3t_jax(so3t)
        point_r = R @ point_3d + t
        point_c = K @ point_r
        point_proj = point_c[0:2] / point_c[2:3]
        residual = point_proj - point_2d
        # return residual
        delta = 2.0
        return jnp.sign(residual)*jnp.sqrt(2.0*delta**2*(jnp.sqrt((residual/delta)**2+1.0)-0.999999))
        qr_residual = jnp.sign(residual) * jnp.sqrt(delta*jnp.fmax(2.0*jnp.abs(residual)-delta, 0.0))
        return residual + (qr_residual-residual) * (0.5+0.5*jnp.sign(jnp.abs(residual)-delta))

    fun_jax = jax.jit(jax.vmap(fun_jax_0))
    fun_jac_jax = jax.jit(jax.vmap(jax.jacfwd(fun_jax_0, [0, 1])))

    def fun(params):
        # poses = params[:6*n_pose].reshape((n_pose, 6))[poses_i]
        # points_3d = params[6*n_pose:].reshape((n_point, 3))[points_i]
        # return np.array(fun_jax(poses, points_3d, points_2d)).flatten()
        ...
        # return residual
        delta = 2
        return np.sign(residual)*np.sqrt(2*delta**2*(np.sqrt((residual/delta)**2+1)-1))
    ...
    def fun_jac(params):
        ...
        poses = params[:6*n_pose].reshape((n_pose, 6))[poses_i]
        points_3d = params[6*n_pose:].reshape((n_point, 3))[points_i]
        jac_poses, jac_points = fun_jac_jax(poses, points_3d, points_2d)
        jac_poses = np.array(jac_poses)
        jac_points = np.array(jac_points)

        # unwrap sparse matrix
        time0 = perf_counter()
        # jac = scipy.sparse.lil_matrix(
        #     (2*n_obs, 6*n_pose+3*n_point), dtype=np.float32)
        # for i, (pose_i, point_i, uv) in enumerate(observations):
        #     p0 = 6*pose_i
        #     jac[2*i:2*i+2, p0:p0+6] = jac_poses[i]
        #     p0 = 6*n_pose+3*point_i
        #     jac[2*i:2*i+2, p0:p0+3] = jac_points[i]
        ...
        for i, (pose_i, point_i, uv) in enumerate(observations):
            for di in range(2):
                ...
                for dj in range(6):
                    ci = 6*pose_i+dj
                    data.append(jac_poses[i,di,dj])
                    row_ind.append(ri)
                    col_ind.append(ci)
                for dj in range(3):
                    ci = 6*n_pose+3*point_i+dj
                    data.append(jac_points[i,di,dj])
                    row_ind.append(ri)
                    col_ind.append(ci)
        jac = scipy.sparse.csr_matrix((data, (row_ind, col_ind)))
        time1 = perf_counter()
        timer += time1-time0
        return jac
    ...
    print('timer: {:.1f} ms'.format(1000*timer))
...
def bundle_adjustment_02(poses, points, observations):
    ...
    # function
    def fun_jax_0(camera, so3t, point_3d, point_2d):
        f, cx, cy = camera
    ...
    fun_jax = jax.jit(jax.vmap(fun_jax_0))
    fun_jac_jax = jax.jit(jax.vmap(jax.jacfwd(fun_jax_0, [0, 1, 2])))
    ...
    def fun_jac(params):
        ...
        # compute jacobian
        camera = jnp.array([params[:3]]).repeat(n_obs, axis=0)
        poses = params[3:3+6*n_pose].reshape((n_pose, 6))[poses_i]
        points_3d = params[3+6*n_pose:].reshape((n_point, 3))[points_i]
        jac_camera, jac_poses, jac_points = fun_jac_jax(camera, poses, points_3d, points_2d)
        jac_camera = np.array(jac_camera)
        jac_poses = np.array(jac_poses)
        jac_points = np.array(jac_points)
        ...
        for i, (pose_i, point_i, uv) in enumerate(observations):
            for di in range(2):
                ...
                for dj in range(3):
                    ci = dj
                    data.append(jac_camera[i,di,dj])
                    row_ind.append(ri)
                    col_ind.append(ci)
                for dj in range(6):
                    ci = 3+6*pose_i+dj
...
if __name__ == "__main__":
    ...
    # bundle adjustment
    time0 = perf_counter()
    print(len(observations), 'observations for BA')
    poses, points = bundle_adjustment_02(poses, points, observations)
    time1 = perf_counter()
